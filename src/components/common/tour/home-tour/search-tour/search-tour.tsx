    ...
    <SectionInViewRight>
      <div className='h-40 px-32 py-3 mb-20 rounded-2xl'>
        <div className='w-full '>
          <h2 className='mb-8 text-2xl font-medium dark:text-white'>Your recent searches</h2>
            ...
            {getAll?.data.map((item: TourResponseType) => (
              <div className='flex gap-2 tour grid-container' key={item.id}>
                ...
                <div className='flex items-center'>
                  <div>
                    <h3 className='overflow-hidden text-lg font-medium whitespace-pre-line w-52 text-ellipsis line-clamp-2 dark:text-white'>
                      {item.description}
                    </h3>
                    <p className='overflow-hidden whitespace-pre-line text-slate-400 w-52 text-ellipsis line-clamp-1 dark:text-gray-400'>
                      {item.name}
                    </p>
                  </div>
                </div>
