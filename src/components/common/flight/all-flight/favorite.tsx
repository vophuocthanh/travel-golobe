...
export default function Favorite({ id }: FavoriteProps) {
  ...
  return (
    <div
      className={`flex items-center justify-center w-10 h-10 text-xs font-medium transition-colors border rounded cursor-pointer border-primary dark:text-white`}
      onClick={handleClick}
    >
      <HeartIcon className={`w-4 h-4 ${isFavorited ? 'text-red-600' : 'text-gray-800'}`} />
    </div>
  )
}
