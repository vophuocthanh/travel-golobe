...
import { CommentTour } from '@/shared/ts/interface/comment-tour.interface'
...
export default function TourDetailReview({ data }: TourDetailReviewProps) {
    ...
    const mutationReview = useMutation({
        mutationFn: ({ tourId, content, rating }: { tourId: string; content: string; rating: number }) =>
            commentTourApi.addComment(tourId, content, rating),
        onSuccess: () => {
            setComment("");
            setRating(0);
        },
